...
static uint8_t sd_get_cardinfo(SD_CardInfo *cardinfo)
{
	...
	{
		printk("[SDCard] %s | sd_get_csdregister failed\r\n",__func__);
		return 0xFF;
	}
	if (sd_get_cidregister(&(cardinfo->SD_cid)))
	{
		printk("[SDCard] %s | sd_get_cidregister failed\r\n",__func__);
		return 0xFF;
	}
	if(2 == sd_version)
	{
...
uint8_t sd_init(void)
{
	...
	while(index--){
		...
		sd_read_data(frame, 4);
		sd_end_cmd();
		debug_print("[SDCard] %s |  frame[0] = %x \r\n",__func__,frame[0]);
		debug_print("[SDCard] %s |  frame[1] = %x \r\n",__func__,frame[1]);
		debug_print("[SDCard] %s |  frame[2] = %x \r\n",__func__,frame[2]);
		debug_print("[SDCard] %s |  frame[3] = %x \r\n",__func__,frame[3]);
		debug_print("[SDCard] %s |  result = %d \r\n",__func__,result);
		debug_print("[SDCard] %s |  index = %d \r\n",__func__,index);
		if(result == 0){
			break;
		}
