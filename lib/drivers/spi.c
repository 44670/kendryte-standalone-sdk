...
#define malloc(x) (printf("spi driver nomalloc: %p\n", __builtin_return_address(0)), assert(0), (void*)0)

volatile spi_t* spiPendingSpiHandle = 0;
dmac_channel_number_t spiPrevDmaChannelNum = 0;

void spiWaitPrevDmaDone() {
	if (spiPendingSpiHandle) {
        dmac_wait_done(spiPrevDmaChannelNum);
		while ((spiPendingSpiHandle->sr & 0x05) != 0x04)
			;
		spiPendingSpiHandle->ser = 0x00;
		spiPendingSpiHandle->ssienr = 0x00;
        spiPendingSpiHandle = 0;
	}
}

volatile spi_t* const spi[4] = { (volatile spi_t*) SPI0_BASE_ADDR, (volatile spi_t*) SPI1_BASE_ADDR,
...
void spi_send_data_normal(spi_device_num_t spi_num, spi_chip_select_t chip_select, const uint8_t* tx_buff,
                          size_t tx_len) {
...
void spi_send_data_normal_dma(dmac_channel_number_t channel_num, spi_device_num_t spi_num,
                              spi_chip_select_t chip_select, const void* tx_buff, size_t tx_len,
                              spi_transfer_width_t spi_transfer_width) {
    spiWaitPrevDmaDone();

	static int havePendingXfer = 0;
    uint32_t* buf = 0;
	configASSERT(spi_num < SPI_DEVICE_MAX && spi_num != 2);
	spi_set_tmod(spi_num, SPI_TMOD_TRANS);
	volatile spi_t* spi_handle = spi[spi_num];
	...
	spi_handle->ser = 1U << chip_select;
    spiPrevDmaChannelNum = channel_num;
    spiPendingSpiHandle = spi_handle;
...
void spi_dup_send_receive_data_dma(dmac_channel_number_t dma_send_channel_num,
                                   ...
                                   spi_chip_select_t chip_select, const uint8_t* tx_buf, size_t tx_len, uint8_t* rx_buf,
                                   size_t rx_len) {
	...
	dmac_set_single_mode(dma_receive_channel_num, (void*) (&spi_handle->dr[0]), v_rx_buf, DMAC_ADDR_NOCHANGE,
	                     DMAC_ADDR_INCREMENT, DMAC_MSIZE_1, DMAC_TRANS_WIDTH_32, v_max_len);

	dmac_set_single_mode(dma_send_channel_num, v_tx_buf, (void*) (&spi_handle->dr[0]), DMAC_ADDR_INCREMENT,
	                     DMAC_ADDR_NOCHANGE, DMAC_MSIZE_4, DMAC_TRANS_WIDTH_32, v_max_len);
...
void spi_receive_data_normal_dma(dmac_channel_number_t dma_send_channel_num,
                                 dmac_channel_number_t dma_receive_channel_num, spi_device_num_t spi_num,
                                 spi_chip_select_t chip_select, const void* cmd_buff, size_t cmd_len, void* rx_buff,
                                 size_t rx_len) {
	configASSERT(spi_num < SPI_DEVICE_MAX && spi_num != 2);
	...
	dmac_set_single_mode(dma_receive_channel_num, (void*) (&spi_handle->dr[0]), rx_buff, DMAC_ADDR_NOCHANGE,
	                     DMAC_ADDR_INCREMENT, DMAC_MSIZE_1, DMAC_TRANS_WIDTH_32, rx_len);
	if (cmd_len)
		dmac_set_single_mode(dma_send_channel_num, cmd_buff, (void*) (&spi_handle->dr[0]), DMAC_ADDR_INCREMENT,
		                     DMAC_ADDR_NOCHANGE, DMAC_MSIZE_4, DMAC_TRANS_WIDTH_32, cmd_len);
...
void spi_receive_data_standard_dma(dmac_channel_number_t dma_send_channel_num,
	...
	spi_receive_data_normal_dma(dma_send_channel_num, dma_receive_channel_num, spi_num, chip_select, write_cmd,
	                            v_cmd_len, read_buf, v_recv_len);
...
void spi_receive_data_multiple_dma(dmac_channel_number_t dma_send_channel_num,
	...
	if (frame_width == SPI_TRANS_INT)
		spi_receive_data_normal_dma(dma_send_channel_num, dma_receive_channel_num, spi_num, chip_select, cmd_buff,
		                            cmd_len, rx_buff, v_recv_len);
	else
		spi_receive_data_normal_dma(dma_send_channel_num, dma_receive_channel_num, spi_num, chip_select, write_cmd,
		                            cmd_len, read_buf, v_recv_len);
...
static void spi_slave_idle_mode(void) {
	...
	spi_handle->ssienr = 0x00;
	spi_handle->ctrlr0 = (0x0 << g_instance.work_mode) | (0x1 << g_instance.slv_oe) |
	    ((g_instance.data_bit_length - 1) << g_instance.dfs);
...
static void spi_slave_command_mode(void) {
	...
	if (g_instance.command.cmd == WRITE_CONFIG) {
		...
		spi_handle->ctrlr0 = (0x0 << g_instance.work_mode) | (0x0 << g_instance.slv_oe) |
		    ((g_instance.data_bit_length - 1) << g_instance.dfs);
		...
		dmac_set_single_mode(g_instance.dmac_channel, (void*) (&spi_handle->dr[0]),
		                     (void*) ((uintptr_t) g_instance.command.addr & 0xFFFFFFF0), DMAC_ADDR_NOCHANGE,
		                     DMAC_ADDR_INCREMENT, DMAC_MSIZE_4, DMAC_TRANS_WIDTH_32, g_instance.command.len * 4);
		...
		dmac_set_single_mode(g_instance.dmac_channel, (void*) ((uintptr_t) g_instance.command.addr & 0xFFFFFFF0),
		                     (void*) (&spi_handle->dr[0]), DMAC_ADDR_INCREMENT, DMAC_ADDR_NOCHANGE, DMAC_MSIZE_4,
		                     DMAC_TRANS_WIDTH_32, g_instance.command.len * 4);
	} else {
		spi_slave_idle_mode();
		return;
	}
...
void spi_slave_config(uint8_t int_pin, uint8_t ready_pin, dmac_channel_number_t dmac_channel, size_t data_bit_length,
                      uint8_t* data, uint32_t len, spi_slave_receive_callback_t callback) {
	g_instance.status = IDLE;
	...
	spi_handle->ssienr = 0x00;
	spi_handle->ctrlr0 =
	    (0x0 << g_instance.work_mode) | (0x1 << g_instance.slv_oe) | ((data_bit_length - 1) << g_instance.dfs);
